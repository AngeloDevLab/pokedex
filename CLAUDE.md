...
- `nav.js` — the burger + sidebar nav shell shared by every page, plus the desktop inline header nav (see CSS section below). `NAV_LINKS` (`{ top: [...], categories: [...] }`, since the Zwischenschritt before Session 12) is the single source of truth for which tool pages appear in both — one flat top-level link (Search) plus category groups (currently Rechner, Pokémon-Guide; a Referenz category will be added once Session 13 ships the first reference page — no empty dropdown on Vorrat). `initNavShell()` injects sidebar markup (top link + `<details>`/`<summary>` accordions per category + language toggle) into `<div id="sidebar">`, and the equivalent header markup (top link + click-to-open `.nav-category` dropdowns) into `#header-nav-slot`/`#header-lang-slot`, then wires the burger toggle, the category-dropdown click/click-outside-to-close (`bindNavCategories()`), and the language toggle. Must run *before* `initI18n()` in each entry script, since `initI18n()` binds/translates the markup `initNavShell()` just injected.
...
- `css/components/` — one file per UI component (`buttons.css`, `inputs.css`, `search.css`, `cards.css`, `dialog.css`, `nav.css`, `stats.css`, `matchup.css`, `learnset.css`, `damage-calc.css`, `breeding.css`, `speed-tiers.css`, `competitive.css`, `forms.css`), flat, no further nesting.
