...
**`js/core/`:**
- `api.js` — all PokéAPI fetch calls. No dependencies on other files. `speciesCache`/`moveCache` memoize species/move lookups; `fetchAllPokemonList()` also persists to `localStorage` (versioned key `pokedex:allPokemonList:v1`) so every tool page's Pokémon picker shares one cached copy.
...
- `pokemon-picker.js` — the Pokémon picker (datalist input + `?pokemon=` deep-link resolution) shared by every tool page (stats calculator, type matchup, learnset, and future Session 9/11/14 pages). `initPokemonPicker({ inputId, datalistId, onSelect })` fetches the cached all-Pokémon list, wires the input, resolves a `?pokemon=` query param if present, and calls `onSelect(pokemon)` with the fully-fetched Pokémon object either way — extracted out of `stats-calc-page.js` once `matchup-page.js` needed the identical logic.
...
- `css/components/` — one file per UI component (`buttons.css`, `inputs.css`, `search.css`, `cards.css`, `dialog.css`, `nav.css`, `stats.css`, `matchup.css`, `learnset.css`), flat, no further nesting.
